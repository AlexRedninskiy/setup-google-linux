...
if [ $linux = "-u" ] ; then
  ...
  wget https://github.com/AlexRedninskiy/setup-google-linux/blob/master/doubuntu.sh
  sh doubuntu.sh $username $password
  rm doubuntu.sh
elif [ $linux = "-d" ] ; then
  echo 'Now configuring your Debian.'
  wget https://github.com/AlexRedninskiy/setup-google-linux/blob/master/dodebian.sh
